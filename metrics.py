...
def carbon_frequency_split(frequently_ordered: pd.DataFrame) -> tuple:
    """
    Split the frequently-ordered dishes into a high-carbon and a
    low-carbon half (median split on co2e_per_cover), each sorted by
    order frequency (covers_sold desc) rather than by carbon. Join key
    used: dish_id -> sku -> emission_factor (co2e_per_cover already on
    the input table from dish_carbon_table).
    """
    median_co2e = frequently_ordered["co2e_per_cover"].median()
    high_carbon = frequently_ordered[frequently_ordered["co2e_per_cover"] >= median_co2e].sort_values("covers_sold", ascending=False)
    low_carbon = frequently_ordered[frequently_ordered["co2e_per_cover"] < median_co2e].sort_values("covers_sold", ascending=False)
    return high_carbon, low_carbon
...
def members_who_chose_lower_impact(full_joined: pd.DataFrame, dim: str = None) -> dict:
    """
    Unique full/racing members who have ordered at least one lower-impact
    dish (member-level, not cover-level). Join keys used: member_id ->
    membership_type (member_profile), dish_id -> lower_impact_flag
    (recipe_master). Optionally broken down by one demographic dimension.
    """
    df = full_joined[full_joined["member_matched"] & full_joined["membership_type"].notna() & full_joined["dish_matched"]]
    total_members = df["member_id"].nunique()
    adopters = df.loc[df["lower_impact_flag"] == True, "member_id"].nunique()  # noqa: E712
    pct = (adopters / total_members * 100) if total_members else 0.0
    result = {"total_members": int(total_members), "adopters": int(adopters), "pct": pct}

    if dim:
        by_dim_total = df.groupby(dim)["member_id"].nunique()
        by_dim_adopt = df.loc[df["lower_impact_flag"] == True].groupby(dim)["member_id"].nunique()  # noqa: E712
        seg = (by_dim_adopt / by_dim_total * 100).fillna(0).round(1).reset_index(name="pct_members_adopted")
        result["by_dim"] = seg.sort_values("pct_members_adopted", ascending=False)
    return result
